...
def do_giana_eval(path_to_input, path_to_gt, path_to_output, team, thr=0, series=False):

    # DEBUGGING !!!!!
    nvids = 18  # should be 18
    
    path_to_input_detection = os.path.join(params.res, "Detection")
    path_to_input_detection = os.path.join(path_to_input_detection, team)
    path_to_input_localization = os.path.join(params.res, "Localization")
    path_to_input_localization = os.path.join(path_to_input_localization, team)

    path_to_output_detection = os.path.join(path_to_output, "Detection/"+team)
    path_to_output_localization = os.path.join(path_to_output, "Localization/"+team)
    folder_output_classif = os.path.join(path_to_output, "Classif/"+team)
    ...
    if not os.path.exists(path_to_output_detection):
        os.makedirs(path_to_output_detection)
    if not os.path.exists(path_to_output_localization):
        os.makedirs(path_to_output_localization)
    if not os.path.exists(folder_output_classif):
        os.makedirs(folder_output_classif)

    files_detection = sorted(os.listdir(path_to_input_detection))[0:nvids]
    files_localization = sorted(os.listdir(path_to_input_localization))[0:nvids]
    ...
    for detection, localization in zip(files_detection, files_localization):
        ...
        detection_csv = os.path.join(path_to_input_detection, detection)
        detection_df = pd.read_csv(detection_csv, header=None)
        detection_confidence = detection_df.shape[1] > 2

        localization_csv = os.path.join(path_to_input_localization, localization)
        ...
        pd.DataFrame.from_dict(res_detection, columns=["TP", "FP", "TN", "FN", "RT"], orient='index').to_csv(
            os.path.join(path_to_output_detection, "d{}.csv".format(vid_name)))
        ...
        pd.DataFrame.from_dict(res_localization, columns=["TP", "FP", "TN", "FN", "RT"], orient='index').to_csv(
            os.path.join(path_to_output_localization, "l{}.csv".format(vid_name)))
    ...
    path_to_output_average = os.path.join(path_to_output, "Averages/"+team)

    calculate_average_results(results_detection, thresholds, os.path.join(path_to_output_average,"detection.csv"))
    calculate_average_results(results_localization, thresholds, os.path.join(path_to_output_average,"localization.csv"))
    calculate_average_classif_results(results_classif, thresholds, os.path.join(path_to_output_average,"classification.csv"))
    ...
    detframe.to_csv(os.path.join(path_to_output_detection, "detection.csv"))
    locframe.to_csv(os.path.join(path_to_output_localization, "localization.csv"))
...
if __name__ == '__main__':
    ...
    do_giana_eval(params.res, folder_gt, output_folder, team, params.thr, params.list)
